...
x = (data[:, 0] - data[0, 0]).reshape(
    N, 1
) / 4  # Shift and rescale the input for visualization purposes
...
plt.savefig("Olympic_dataset.png", bbox_inches="tight")

# **Exercise 2c)**
sigma_square = 10
mu_0 = np.array([[0], [0]])
Sigma_0 = np.array([[100, 0], [0, 5]])


def posterior(X, t, Sigma_0, mu_0):
    Sigma = np.linalg.inv(1 / sigma_square * X.T @ X + np.linalg.inv(Sigma_0))
    mu = Sigma @ (1 / sigma_square * X.T @ t + np.linalg.inv(Sigma_0) @ mu_0)
    return mu, Sigma


# **Exercise 2d)**

muw, Sigmaw = posterior(X, t, Sigma_0, mu_0)
# muw = np.zeros((2, 1))
...
plt.savefig("Visualize_posterior.png", bbox_inches="tight")
plt.show()
